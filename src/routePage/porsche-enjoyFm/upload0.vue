          ...
          <div class="porsche-upload-containerdiv" v-for="(list,key) in uploadcontainerlist">
            <div class="upload-container upload-container-zw">
                <div class="upload-container-tjfm" style="width:280px">
                    ...
                    <div class="img-nrj" v-if="imgnrj">
                      <img width="100%" class="cover" :neme="key" @click="delectimg(key)" :src="PORSCHE_HOST+list.imgpic" alt="">
                    </div>
                </div>
                <div class="upload-container-title upload-container-titlezw">
                    <!-- <h5>正文</h5> -->
                    <textarea  name="name" v-model="list.content" placeholder="正文" rows="5" cols="100" ></textarea>
                </div>
            </div>
          </div>
          ...
          <div class="porsche-upload-container" v-show="uploadcontainer">
            <div class="porsche-upload-containerdiv upload-container-tjfmapp">
              <template v-for="(list,key) in uploadcontainerlist" v-if="list.imgpic != ''">
                <div @click="showTextarea(key,list.id)" :style="comentHeight" class="comment-images-container image-item" v-if="list.imgpic != '' " :class="{'checkeds':list.active,'btn-arrow':list.active && key <= 4, 'btn-arrows':list.active && key > 4}">
                  <span  @click="imgSrclick(key)">
                      <img width="100%" @click="delectimgapp(key)"  class="cover lazy " style="display: inline;" :neme="key"  :src="PORSCHE_HOST+list.imgpic" alt="">
                  </span>
                ...
                <div class="upload-container-title upload-container-titlezw upload-container-title-appa " v-if="isShowTextarea(showthe,key,uploadcontainerlist.length) && uploadcontainerlist.length !=0&&selectId!==''">
                    <textarea placeholder="正文"  name="name" v-model="uploadcontainerlist[selectId]['content']"   rows="5" cols="100"></textarea>
                </div>
              </template>
...
export default {
  ...
  methods: {
    ...
    share() {
      ...
      if( _this.uploadcontainerlist[0].content === "" || !contentNo.test( _this.uploadcontainerlist[0].content) || _this.uploadcontainerlist[0].imgpic === "" ||  _this.title === "" ||  _this.picData.toString() === ""){
        alert("内容不能为空")
        return false;
      }
